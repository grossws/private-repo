...
| `nexusPrivatePluginsBootstrap` | Comma-separated list of manifest `[repoId/]GAV` coordinates to add plugins to `pluginManagement`; default repoId is `gradle` | empty | `com.example:bootstrap:1.0.0, releases/com.example.platforms:platforms:1.5.0`
...
pluginManagement {
  ...
  plugins {
    ...
    // So for manifest:
    //   ids = com.example.first, com.example.second
    //   version = 1.2
